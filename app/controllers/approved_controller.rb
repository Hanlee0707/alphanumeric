class ApprovedController < ApplicationController
  ...
  def index
    ...
    if params[:editor_id] and current_employee.id.to_s == params[:editor_id]
      @isEditor = true
      @articles = Article.where("editor_id=? and status = ?", params[:editor_id], "Approved").paginate page: params[:page], order: 'created_at desc', per_page: 20
    elsif params[:contributor_id] and current_employee.id.to_s == params[:contributor_id]
      @isContributor = true
      @articles = Article.where("contributor_id=? and status = ?", params[:contributor_id], "Approved").paginate page: params[:page], order: 'created_at desc', per_page: 20
    else
