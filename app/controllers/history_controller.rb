class HistoryController < ApplicationController
  ...
  def editor
    ...
    if employee_privilege("Editor") == false then
      ...
      @articles = Article.where("editor_id=? and status = ?", current_employee.id, "Archived").paginate page: params[:page], order: 'published_at desc', per_page: 20
  ...
  def contributor
    ...
    if employee_privilege("Contributor") == false then
      ...
      @articles = Article.where("contributor_id=? and (status = ? or status = ? or status = ?)", current_employee.id, "Approved", "Published", "Archived").paginate page: params[:page], order: 'published_at desc', per_page: 20     
